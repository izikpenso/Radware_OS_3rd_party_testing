...
sshpass -p $VM_SSH_PASSWORD ssh -o "StrictHostKeyChecking no" $VM_SSH_USER@$VM_IP 'cd /opt/stack/neutron-lbaas/neutron_lbaas/; echo "sudo python -m testtools.run neutron_lbaas.tests.tempest.v2.api.radware_test_members.RadwareMembersTest.test_add_member" > ~/lbaas_v1_tempest_tests.log; sudo python -m testtools.run neutron_lbaas.tests.tempest.v2.api.radware_test_members.RadwareMembersTest.test_add_member >> ~/lbaas_v2_tempest_tests.log 2>&1'
