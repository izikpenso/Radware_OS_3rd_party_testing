...
sshpass -p $VM_SSH_PASSWORD ssh -o "StrictHostKeyChecking no" $VM_SSH_USER@$VM_IP 'cd /opt/stack/neutron-lbaas/neutron_lbaas/; echo "tox -r -e tempest" > ~/lbaas_v2_tempest_tests.log 2>&1; sudo tox -r -e tempest >> ~/lbaas_v2_tempest_tests.log 2>&1'
