...
sshpass -p $VM_SSH_PASSWORD ssh -o "StrictHostKeyChecking no" $VM_SSH_USER@$VM_IP 'echo 'HOST_IP="$VM_IP"' >> ~/devstack/localrc; echo FLAT_INTERFACE=eth0 >> ~/devstack/localrc ; echo 'NEUTRON_BRANCH="$NEUTRON_GERRIT_REFSPEC"' >> ~/devstack/localrc; echo 'TEMPEST_BRANCH="$TEMPEST_GERRIT_REFSPEC"' >> ~/devstack/localrc; echo 'NOVA_BRANCH="$NOVA_GERRIT_REFSPEC"' >> ~/devstack/localrc;'
