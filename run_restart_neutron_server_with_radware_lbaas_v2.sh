...
sshpass -p $VM_SSH_PASSWORD scp restart_neutron_server_with_radware_lbaas_v2.sh $VM_SSH_USER@$VM_IP:~/scripts/.

sshpass -p $VM_SSH_PASSWORD ssh -o "StrictHostKeyChecking no" $VM_SSH_USER@$VM_IP 'chmod a+x ~/scripts/restart_neutron_server_with_radware_lbaas_v2.sh; ~/scripts/restart_neutron_server_with_radware_lbaas_v2.sh'
