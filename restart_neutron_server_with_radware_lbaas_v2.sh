...
sed -i "s/# service_provider = LOADBALANCERV2:LoggingNoop/service_provider = LOADBALANCERV2:LoggingNoop/g" /etc/neutron/neutron_lbaas.conf

#sed -i "s/# service_plugins = neutron_lbaas.services.loadbalancer.plugin.LoadBalancerPlugin/service_plugins = neutron_lbaas.services.loadbalancer.plugin.LoadBalancerPluginv2/g" /etc/neutron/neutron.conf
