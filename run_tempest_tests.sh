...
sshpass -p $VM_SSH_PASSWORD ssh -o "StrictHostKeyChecking no" $VM_SSH_USER@$VM_IP 'cd; cd devstack; source openrc admin demo; nova service-list >> ~/lbaas_v1_tempest_tests.log'
sshpass -p $VM_SSH_PASSWORD ssh -o "StrictHostKeyChecking no" $VM_SSH_USER@$VM_IP 'sudo pip install tempest-lib; cd /opt/stack/neutron-lbaas/neutron_lbaas/; echo "tox -e apiv1 neutron_lbaas.tests.tempest.v1.api.test_radware_load_balancer.RadwareLoadBalancerTest.test_create_update_delete_pool_vip" > ~/lbaas_v1_tempest_tests.log; tox -e apiv1 neutron_lbaas.tests.tempest.v1.api.test_radware_load_balancer.RadwareLoadBalancerTest.test_create_update_delete_pool_vip >> ~/lbaas_v1_tempest_tests.log 2>&1'
