...
#sshpass -p $VM_SSH_PASSWORD ssh -o "StrictHostKeyChecking no" $VM_SSH_USER@$VM_IP 'sed -i "s/while time.time() - start_time <= timeout:/while time.time() - start_time <= 3000:/g" /opt/stack/neutron-lbaas/neutron_lbaas/tests/tempest/lib/services/network/json/network_client.py; sed -i "s/allow_tenant_isolation = True/allow_tenant_isolation = False/g" /opt/stack/tempest/etc/tempest.conf'
#sshpass -p $VM_SSH_PASSWORD ssh -o "StrictHostKeyChecking no" $VM_SSH_USER@$VM_IP 'sed -i "s/if int(time.time()) - start_time >= self.build_timeout:/if int(time.time()) - start_time >= 3000:/g" /opt/stack/neutron-lbaas/neutron_lbaas/tests/tempest/lib/services/network/json/network_client.py;'
